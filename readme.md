...
Теперь количество выполненных команд в этой функции составляет всего `1.8 миллиардов`, это в *7.25* раз меньше, а значит гораздо быстрее. В свою очередь количество команд в функции поиска упало с `20 миллиардов` до `7.6 миллиардов`. А время сократилось с `131 * 10^8`  `76 * 10^8` тиков.
...
В результате этой оптимизации количество операций сократилось с `7.6 миллиардов` до `6.1 миллиардов`, а время уменьшилось с `76 * 10^8` до `65 * 10^8` тиков, что является хорошим результатом.
...
Как можно заметить, количество операций в функции `find_in_hash_table()` снизилось с `6.1 миллиардов` миллионов до `4.8 миллиардов`, а время снизилось с `65 * 10^8` до `59 * 10^8` тиков. Что только подтверждает важность учёта накладных расходов при проектировании приложения.
