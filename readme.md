...
![no_optimization](img/no_optimization_graph.png)

Как можно заметить функция поиска вызывает 2 функции:
+ Подсчёта хеша
+ Линейный поиск по списку

Причём собственное время выполнения функции по сравнению с этими двумя ничтожно мало.

## Intrinsic оптимизация

Начнём оптимизацию с самого очевидного места - **хеш функции**, потому что время её подсчёта составляет большую часть от времени выполнения функции поиска.

До оптимизации количество выполненных команд в функции подсчёта хеша составляет `29 миллионов` 

Используем intrinsic функцию стандарта SSE4.2 - `_mm_crc32_u8()` в качестве функции подсчёта хеша.
Заменим ей внутренний цикл подсчёта хеша в наивном алгоритме:
...
![intc_opt](img/intrinsic_optimization.png)

Как видно время выполнения функции подсчёта хеша значительно сократилось и теперь составляет всего `20%` от всего времени выполнения функции поиска.

Теперь количество выполненных команд в функции подсчёта хеша составляет всего `4 миллиона`, это в *7.25* раз меньше, а значит гораздо быстрее. 
...
Заметим, что длина 1 строки в нашем наборе данных не превышает 32 символа, а значит можно поместить каждое слово в ячейки по 32 байта и сравнивать области памяти, а не строки разной длины. 